    ...
    if distance == 'l_inf':
        for _ in range(perturb_steps):
            ...
            with torch.enable_grad():
                if method != 'trades':
                    loss_ce = F.cross_entropy(model(x_adv, _eval=True), y)
                else:
                    loss_ce = criterion_kl(F.log_softmax(model(x_adv, _eval=True), dim=1),
                                           F.softmax(model(x_natural, _eval=True), dim=1)) 
