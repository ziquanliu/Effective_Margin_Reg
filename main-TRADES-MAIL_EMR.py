...
class Trainer():
    ...
    def train(self, model, tr_loader, va_loader=None, adv_train=False):
        ...
        scheduler = torch.optim.lr_scheduler.MultiStepLR(opt, 
                                                         milestones=[iter_per_epoch*75, iter_per_epoch*90], 
                                                         gamma=0.1)
        _iter = 0
        ...
        for epoch in range(1, args.max_epoch+1):
            ...
            for data, label in tr_loader:
                ...
                if epoch <=74:
                    ...
                    loss, output, adv_data = MAIL.mail_loss(model, data, label, opt, bias = 0.05, slope = 5., beta=5.0, method = 'trades')
                ...
                (logit_grad_norm_decay*logit_grad_l2_norm + loss).backward()
                opt.step()
            ...
            if va_loader is not None:
                ...
                va_acc, va_adv_acc = va_acc * 100.0, va_adv_acc * 100.0
                if va_adv_acc>best_robust_acc:
                    best_robust_acc = va_adv_acc
